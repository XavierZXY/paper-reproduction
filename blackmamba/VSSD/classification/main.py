...
def parse_option():
    ...
    parser.add_argument(
        "--ddp", type=str, default="torch", help="distributed data parallel"
    )
    parser.add_argument(
        "--enable_preload",
        action="store_true",
        help="tricks to fix the bug of DataLoader",
    )
    parser.add_argument(
        "--enable_persistance",
        action="store_true",
    ...
    parser.add_argument(
        "--mesa",
        action="store_true",
        help="tricks to prevent overfitting, following MLLA",
    )
    parser.add_argument(
        "--mesa_value",
        type=float,
...
def main(config, args):
        ...
        if dist.get_rank() == 0:
            if hasattr(model, "flops"):
                ...
                if args.ddp == "torch":
                    logger.info(
                        flop_count_str(
                            FlopCountAnalysis(model, (dataset_val[0][0][None],))
                        )
                    )
    model.cuda()
    ...
    if config.MODEL.RESUME:
        max_accuracy, max_accuracy_ema, steps = load_checkpoint_ema(
            config,
            model_without_ddp,
            optimizer,
            lr_scheduler,
        ...
        if config.EVAL_MODE:
            ...
            if model_ema is not None:
                ...
                logger.info(
                    f"Accuracy of the network ema on the {len(dataset_val)} test images: {acc1_ema:.1f}%"
                )
            return
    ...
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        ...
        train_one_epoch(
            ...
            tb_writer=writer,
            mesa=args.mesa_value
            if (epoch >= int(0.25 * config.TRAIN.EPOCHS) and args.mesa)
            else -1.0,
        )
        steps = 0
        if dist.get_rank() == 0:
...
def throughput(data_loader, model, logger):
    ...
    for idx, (images, _) in enumerate(data_loader):
        ...
        logger.info(
            f"batch_size {batch_size} mem cost {torch.cuda.max_memory_allocated() / 1024 / 1024} MB"
        )
        return
