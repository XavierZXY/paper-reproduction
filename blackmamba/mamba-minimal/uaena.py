...
class ResidualBlock(nn.Module):
    def __init__(self, args: ModelArgs):
        """Simple block wrapping Mamba block with normlization and residual connection.
...
class MambaBlock(nn.Module):
    ...
    def forward(self, x):
        """Mamba block forward method.

        Args:
            x (_type_): (b, l, d)
        """
        (b, l, d) = x.shape
        x_and_res = self.in_proj(x)  # (b, l, d_in * 2)
        (x, res) = x_and_res.split(
            split_size=[self.args.d_inner, self.args.d_inner],
            dim=-1,
        )
    ...
    def ssm(self, x):
        ...
        (delta, B, C) = x_dbl.split(
            split_size=[self.args.dt_rank, n, n],
            dim=-1,
        )
