...
def get_network(args):
    ...
    if args.fbs:
        # use fbs
        from models.resnet import resnet18_fbs
...
def compute_mean_std(cifar10_dataset):
    ...
    data_r = numpy.dstack(
        [cifar10_dataset[i][1][:, :, 0] for i in range(len(cifar10_dataset))]
    )
    data_g = numpy.dstack(
        [cifar10_dataset[i][1][:, :, 1] for i in range(len(cifar10_dataset))]
    )
    data_b = numpy.dstack(
        [cifar10_dataset[i][1][:, :, 2] for i in range(len(cifar10_dataset))]
    )
